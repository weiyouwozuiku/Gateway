...
func Test_DBPool(t *testing.T) {
	SetUp()
	// 获取连接池
	dbpool, err := middleware.GetDBPool("default")
	if err != nil {
		t.Fatal(err)
	}
	// 执行事务
	trace := middleware.NewTrace()
	if _, err := middleware.DBPoolQuery(trace, dbpool, beginSQL); err != nil {
		t.Fatal(err)
	}
	// 创建表
	if _, err := middleware.DBPoolQuery(trace, dbpool, createTableSQL); err != nil {
	...
	fmt.Printf("%6s | %6s\n", "id", "created_at")
	for {
		rows, err := middleware.DBPoolQuery(trace, dbpool, selectSQL, current_id)
		if err != nil {
			middleware.DBPoolQuery(trace, dbpool, rollbackSQL)
			t.Fatal(err)
		}
	...
	// 删除表
	if _, err := middleware.DBPoolQuery(trace, dbpool, dropTableSQL); err != nil {
		middleware.DBPoolQuery(trace, dbpool, rollbackSQL)
		t.Fatal(err)
	}
	// 提交事务
	middleware.DBPoolQuery(trace, dbpool, commitSQL)
	TearDown()
}
